...
var GITHUB_API_ENDPOINT = "https://api.bitbucket.org/1.0";
 ...
 */
var save_bitbucket_repo_deploy_keys = exports.save_bitbucket_repo_deploy_keys = function(user_obj, gh_repo_url, privkey, pubkey, repo_name, callback)
{
  console.log("here");
  generate_webhook_secret(function(secret) {
    var config = {};
    config.name = repo_name;
    config.owner = user_obj.bitbucket.username;
    config.privkey = privkey;
    config.pubkey = pubkey;
    config.url = gh_repo_url.toLowerCase();
    config.display_url = gh_repo_url;
    config.secret = secret;
    user_obj.bitbucket_config.push(config);
 ...
 * <callback> function(error, response, body)
 */
var add_deploy_key = exports.add_deploy_key = function(gh_repo_path, pubkey, title, token, user_obj, repo_config, callback, client)
{
  console.log(repo_config);
  console.log(user_obj.bitbucket);

  var data = {key: pubkey};
  var url = GITHUB_API_ENDPOINT + "/repositories/" + repo_config.owner + "/" + repo_config.name + "/deploy-keys";
  ...
  oAuth.post(
    url,
    user_obj.bitbucket.accessToken, user_obj.bitbucket.accessTokenSecret,
    data,
    callback
  );
};
...
var setup_integration = exports.setup_integration = function(user_obj, gh_repo_id, token, callback, socket, client) {
  var gh_metadata = user_obj.bitbucket_metadata[user_obj.bitbucket.username];
  ...
  var keyname = "/tmp/" + user_obj.bitbucket.username + gh_repo_path.replace(/\//g, '_');
  Step(
    function make_keys() {
      socket.emit("update", {msg:"generating deploy keys"});
      ssh.generate_keypair(user_obj.bitbucket.username, keyname, this);
    },
    ...
    function save_keys(err, privkey, pubkey) {
      ...
      if (err) throw err;
      save_bitbucket_repo_deploy_keys(user_obj, config_key, privkey.toString(), pubkey.toString(), repo_name, this);
    },
    function get_repo_config(err, user_obj) {
      user_obj.get_bitbucket_repo_config(config_key, this);
    },
    function push_deploy_key(err, repo_config, access_level, user_obj) {
      if (err) {
      ...
      socket.emit("update", {msg:"sending deploy key to Bitbucket"});
      var title = "StriderDeployKey - " + config.strider_server_name + " - " + user_obj.email;
      add_deploy_key(gh_repo_path, this.repo.pubkey, title, token, user_obj, repo_config, this);
    },
    function set_hook(err, res, body) {
      console.log("setting up hook");
      socket.emit("update", {msg:"configuring secure Bitbucket commit hook"});
...
var setup_integration_manual = exports.setup_integration_manual = function(req, org, github_url, callback) {
  ...
  Step(
    ...
    function save_keys(err, privkey, pubkey) {
      if (err) throw err;
      save_bitbucket_repo_deploy_keys(req.user, config_key, privkey.toString(), pubkey.toString(), this);
    },
    function get_repo_config(err, user_obj) {
      if (err) throw err;
