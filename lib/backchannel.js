  ...
  switch (msg.msg_type) {
    ...
    case 'done':
      Job.findOne({_id: msg.job_id}).populate('_owner').exec(function (err, job) {
        ...
        getRepoConfig(job, function(err, repo_config) {
...
function getRepoConfig(job, callback){
  if (job.vendor == "bitbucket"){
    job._owner.get_bitbucket_repo_config(job.repo_url, callback);
  }
  else {
    job._owner.get_repo_config(job.repo_url, callback);
  }
}
        ...
        websockets.user_msg(msg.userId, 'done',
          {
            ...
            test_exitcode: msg.testExitCode, 
            deploy_exitcode: msg.deployExitCode
          }
        );
        job.save();
      });
