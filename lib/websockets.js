...
exports.init = function(app, server, session_store){
  ...
  sio.sockets.on('connection', function(socket) {
    ...
    if (session.passport === undefined) {
      ...
      socket.on('setupbitrepo', function(data) {
        User.findOne({'_id': session.passport.user}, function (err, foundUser) {
          var kickoff_repo = foundUser.get_bitbucket_repo_metadata(data.repo_id);
          bitbucket.setup_integration(foundUser, kickoff_repo.id, foundUser.get('bitbucket.accessToken'), function() {
            console.log("sending bitrepodone");
            socket.emit("update", {msg:"done"});
            socket.emit("bitrepodone", {});
          }, socket);
        });
      });
    }
