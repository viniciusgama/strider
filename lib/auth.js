...
/* Assign all the various properties coming from github to the user document */
function bitbucketUserLink(ghProfile, ghAccessToken, ghAccessTokenSecret, user) {
  var ghUser = ghProfile._json;
  console.log(ghUser.repositories);
  var bitbucket = {
      username: ghUser.user.username,
      first_name: ghUser.user.first_name,
      last_name: ghUser.user.last_name,
      avatar: ghUser.user.avatar,
      resource_uri: ghUser.user.resource_uri,
      accessToken: ghAccessToken,
      accessTokenSecret: ghAccessTokenSecret
  };
  user.set('bitbucket', bitbucket);
}

var setupBitbucketAuth = function(app){
  passport.use(new BitbucketStrategy({
    consumerKey: config.bitbucket.consumerKey,
    consumerSecret: config.bitbucket.consumerSecret,
    callbackURL: config.bitbucket.callbackURL + "/auth/bitbucket/callback",
    passReqToCallback: true
  },
  function(req, token, tokenSecret, profile, done) {
    if (!req.user){
      return done("Cannot sign up with bitbucket - you must link it to account");
    } 
    bitbucketUserLink(profile, token, tokenSecret, req.user);
    req.user.save(function(e){
        if(e) throw e;
        console.log(req.user.bitbucket);
        done(null, req.user);
    });
  }));
}
...
var requireRepoAdmin = function(req, res, next) {
  ...
  if (req.user === undefined) {
    ...
    if (host === "github"){
      ...
      req.user.get_bitbucket_repo_config(repo_url, function(err, repo, access_level) {
        if (err || !repo || access_level < 1) {
          console.log("user '" + req.user.email + "' attempted to view " + repo_url + " without authorization.");
          res.statusCode = 401;
          return res.end("not authorized");
