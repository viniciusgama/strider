...
UserSchema.methods.get_bitbucket_repo_config = function(case_insensitive_url, cb) {
    if (!cb) {
        throw new Error("No callback supplied");
    }
    // If the repo is found in the current user, it means we have admin privileges
    var repo = _.find(this.bitbucket_config, function(repo_config) {
        return case_insensitive_url.toLowerCase() == repo_config.url;
    });
    if (repo) {
        return cb(null, repo, 1, this);
    ...
    var self = this;
    console.debug("user.get_bitbucket_repo_config() - no repos found matching %s in %j, querying DB",
        case_insensitive_url, this.bitbucket_config);
    // Look for user object with the project url configured AND current user id is in collaborators list OR repo is public.
    User.findOne({"bitbucket_config":{$elemMatch:{
        "url":case_insensitive_url.toLowerCase(),
        "$or": [
            {"collaborators.user_id": self._id},
            {"public": true}
        ...
        if (err || !user_obj) {
            console.debug("user.get_bitbucket_repo_config() - did not find a repo matching %s for any user",
                case_insensitive_url);
            return cb("no repo found", null);
        }
        ...
        if (!repo) {
            console.error(
                "user.get_bitbucket_repo_config() - Error finding matching github_config despite DB query success!");
            return cb("no repo found", null);
        }
        var c = _.find(repo.collaborators, function(c) { return c.user_id.toString() === self._id.toString();});
        // if this user is a collaborator, we take the access level from the collaborator (highest wins)
        ...
        // otherwise, repo must be marked as public and access_level is still 0 (read-only)
        if (repo.public) {
            console.debug("user.get_bitbucket_repo_config() - not a collaborator on %s but because it's public we allow", case_insensitive_url);
            return cb(null, repo, 0, user_obj);
        }
        // fallthru (should never happen due to query clause)
        console.error(
            "user.get_bitbucket_repo_config() - DANGER WILL ROBINSON. This should never happen. Repo is private and user is not a collaborator. How could this be?");
        return cb("access denied", null);
...
UserSchema.methods.get_bitbucket_repo_config_list = function(cb, access_level) {
  ...
  var repo_list = this.bitbucket_config;
  // Look for any additional configured repositories for which we are listed as access_level;
  var self = this;
  User.find({
    // Ignore our own user object.
    "_id":{$ne:self._id},
    // Our user ID is in the collaborators list.
    "bitbucket_config.collaborators.user_id":self._id,
  }, function(err, results) {
      if (err) {
        throw err;
      }
...
UserSchema.methods.get_bitbucket_repo_metadata = function(id, userId) {
  var bt_user_id = userId || this.bitbucket.username;
  var repository = _.find(this.bitbucket_metadata[bt_user_id], function(repo) {
    return repo.id == id;
  });
  return repository;
};
...
UserSchema.methods.get_bitbucket_prod_deploy_target = function(case_insensitive_url,
    cb) {
    var self = this;
    this.get_bibucket_repo_config(case_insensitive_url, function(err, repo) {
      var deploy_target;
      if (repo.has_prod_deploy_target) {
        // XXX assumes heroku for now
        deploy_target = _.find(self.heroku, function(heroku) {
