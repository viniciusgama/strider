...
var   apres = require('apres')
    ...
    , api_bitbucket = require('../routes/api/bitbucket.js')
...
var init = exports.init = function (config) {
  ...
  app.get('/:host/:org/:repo/latest_build', auth.requireReadAccess, routes_jobs.latest_build);
  app.get('/:host/:org/:repo/', auth.requireReadAccess, routes_jobs.latest_build);
  app.get('/:host/:org/:repo/job/:job_id', auth.requireReadAccess, routes_jobs.job);

  /* Requires admin access to the repository in the path */
  app.get('/:host/:org/:repo/config', auth.requireUser, auth.requireRepoAdmin,
    routes.config);

  app.post('/webhook', routes.webhook_signature);
  app.post('/webhook/:secret',routes.webhook_secret);

  app.get('/kickoff_bitbucket/:bitbucketId', auth.requireUserOr401, routes.kickoff_bitbucket);
  ...
  app.get('/api/bitbucket/repos', auth.requireUserOr401, api_bitbucket.get_repos);
