function init_kickoff() {
    ...
    window.KickoffRouter = Backbone.Router.extend( {
      ...
      bitbucket: function() {
        console.log("bitbucket_select");
        window.SetupApp.model.set('state', 'bitbucket');
      },
    ...
    window.BitbucketModel = Backbone.Model.extend({
      defaults: function() {
        return {
          "private": false,
          "paas": "heroku",
    ...
    window.BitbucketSetup = Backbone.View.extend({
      template: _.template($("#bitbucket-setup").html()),
      collab_template: _.template($("#bitbucket-collaborators-template").html()),
      events : {
        'click .run-action':function(ev) {
          $(".run-action").hide();
          spinner.html(_.template($("#spinner-msg").html(),
            {message:"Starting Bitbucket repository setup..."})).removeClass().addClass("alert alert-info");
          spinner.show();
          ...
          socket.on('bitrepodone', function(data) {
            ...
            $.ajax("/api/jobs/start", {
              data: {url: repo_url, type:"TEST_ONLY", bitbucket:true},
              dataType: "text",
              type: "POST",
              error: function(xhr, ts, e) {
              ...
              success: function(data, ts, xhr) {
                spinner.html('<b>Bithub repository setup complete!</b> The next step is to configure Continuous Deployment.')
                ...
                $(self.el).find("div.bitbucket-main-content").html(self.collab_template());
          ...
          socket.emit("setupbitrepo", {repo_id:repo.id});
        }
      },
      render: function() {
    ...
    model = new BitbucketModel(repo);
    ghView = new BitbucketSetup({model: model});
